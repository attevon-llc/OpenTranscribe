...
- **`canManage={true}` is hardcoded** where the modal renders `CurrentSharesList` — the UI never
  computes share rights. The real rule is `_require_collection_owner` in
  `backend/app/api/endpoints/media_collections.py`: only the **direct owner** may list, create,
  update, or revoke shares, and an `editor` share does **not** confer re-sharing. Opening this modal
  for a collection you don't own 403s on every call.
