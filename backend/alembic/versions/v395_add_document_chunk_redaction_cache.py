...
Design notes:
  ...
  - **No new columns on ``document`` itself** — ``redaction_status`` /
    ``redaction_model_version`` / ``redaction_coverage`` already exist (v393), added
    ahead of time for exactly this. This revision only adds where the *spans* live.

Revision ID: v395_add_document_chunk_redaction_cache
Revises: v394_add_watch_source_file_document_id
...
down_revision = "v394_add_watch_source_file_document_id"
branch_labels = None
depends_on = None
