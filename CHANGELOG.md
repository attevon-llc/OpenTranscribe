...
- **Offline/Windows installers now generate the MinIO encryption key**: `install-offline-package.sh` and `build-windows-installer.sh` left `MINIO_KMS_SECRET_KEY` at its invalid placeholder (with `MINIO_KMS_AUTO_ENCRYPTION=on`), which prevents MinIO from starting; both now generate a real key (and a `FLOWER_PASSWORD`) like the main installer.
