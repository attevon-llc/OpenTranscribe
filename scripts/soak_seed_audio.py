...
def download_one(
    backend_url: str, token: str, f: dict[str, Any], dest_dir: Path, tier: int
) -> Path:
    uuid = f['uuid']
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for done, fut in enumerate(as_completed(futures), start=1):
            ...
            try:
                ...
                print(
                    f'    [FAIL {done}/{len(picks)}] tier {tier} {f["uuid"][:8]}: {e}',
                    file=sys.stderr,
                )
            except Exception as e:  # noqa: BLE001
                print(
                    f'    [FAIL {done}/{len(picks)}] tier {tier} {f["uuid"][:8]}: {type(e).__name__}: {e}',
                    file=sys.stderr,
                )
...
def main() -> int:
    ...
    print(
        f'Target distribution: T1={args.tier1}  T2={args.tier2}  T3={args.tier3}  T4={args.tier4}\n'
    )
    ...
    if not args.dry_run:
        try:
            ...
            print(
                f'ERROR: auth failed against {args.backend_url} — is the prod stack running? ({e})',
                file=sys.stderr,
            )
            return 1
    else:
    ...
    print(
        f'benchmark/test_audio/ now contains {sum(1 for _ in AUDIO_DIR.iterdir())} files, '
