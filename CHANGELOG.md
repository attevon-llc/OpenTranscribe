...
- Migration `v375`; new capability key `chat.rag` (community default: on); 141 new i18n keys across all 12 locales.
...
- **The OIDC surface is renamed `oidc_*`, and `KEYCLOAK_*` keeps working forever
  ...
  all 12 locales, and the routes (`/api/auth/oidc/login`, `/api/auth/oidc/callback`) are all
...
- **Admission control — "does this deployment want you", separately from "are you who you say"**:
  `oidc_allowed_groups` / `oidc_blocked_groups` evaluated against the roles claim (semicolon-
  delimited, because a directory group value is a DN and contains commas; blocked is evaluated
  first and means *denied* — both are config keys, no migration), and `require_account_approval`
  (`v381`), which lands a newly provisioned account — self-registered **or** JIT-provisioned by
  any external IdP — in a `pending` state with an admin queue at `GET`/`POST
  /api/admin/user-approvals`. An empty allow-list admits everyone, so upgrading changes nothing
  until an operator sets one. Refusals return the same generic 401 an unusable token gets, and
  are audited.
...
- **Plural labels rendered as "12 source"** — the keys used i18next v3 `_plural` suffixes on a v25 install, which silently falls back to the singular. Migrated to `_one`/`_other` across all 12 locales.
...
- **A failed speaker save left the new name on screen looking saved.** It is now reverted (`speakerProfile.errorWithLocal`, "changes are saved locally only", replaced by `saveFailedReverted` across all 12 locales; the dead `speakerProfile.localOnly` removed). Bulk save moves from `Promise.all` to `allSettled` and restores only the speakers that failed — one rejection previously discarded every other speaker's outcome.
...
- **Floating preview players unified + seek fixed**: the search and speaker pop-out players now share one `FloatingPreviewPlayer` component (identical chrome, hour-aware timestamps, title + speaker layout — fixing the speaker player's jumbled time display), timestamp jumps wait for `loadedmetadata` so deep seeks no longer restart playback from 0 on slow links, a refreshed presigned URL is hot-swapped preserving position and play state, and the speaker media-preview honors `MEDIA_URL_EXPIRE_SECONDS` (was hardcoded to 1 hour). The search preview also no longer auto-reopens on back-navigation, and its close button is translated in all 12 locales.
...
- **Auth-state test isolation now restores by snapshot, not by hardcoded reset** (#810 partial).
  A shared `teardown_method` in the lockout/session test files was skipped whenever its own
  `setup_method` raised, and otherwise overwrote the cached globals it protects
  (`_redis_client`, `_in_memory_store`, `_store_initialized`, `_last_redis_probe`,
  `_cas_script`, `_cas_script_client`) with a hardcoded default rather than the value observed
  on entry — clobbering whatever a legitimate neighbouring test had left. Extracted into a
  shared snapshot/restore fixture and applied to all five affected files; a real leak surfaced
  in the process (three files reset five of the six globals via `monkeypatch` but omitted
  `_last_redis_probe`, leaving a live timer reading behind for the next test). Does **not**
  close #810 — its CI flake is not reproduced by this mechanism and the underlying failure
  stays open, pointed at a concurrency angle instead.
- **A committed OpenAPI snapshot now gates the CI schema diff** (#798 step 1). `backend/openapi.json`
  is generated by `scripts/generate-openapi.py` and diffed in the existing backend-tests CI job, so
  an accidental breaking API change now shows as a reviewable diff on the PR instead of surfacing
  only when a caller hits it in production. Step 1 only — `/api/v1` and a public stability
...
- **Engine Configuration panel fully internationalized**: all infrastructure keys (titles, backend labels, Save/Reset/Saved, DB/Env/Default source badges) translated across all 12 locales at full key parity. The ASR provider dropdown now flags experimental/untested providers inline.
...
- **Dead LLM display-name helpers removed** (#897): `getProviderDisplayName`/`getProviderDefaults`
  had no remaining callers after the LLM provider settings consolidation; deleted rather than
  left for a future contributor to wonder whether they were load-bearing.
- **CI: docs build now gated behind a PR check** (#793): `deploy-docs.yml` previously ran the
  Docusaurus build only on push to master, so a break was caught by a failed deploy after merge
  rather than before — the docs site is built into a published image. It now also builds (never
  deploys) on `pull_request`, path-filtered to `docs-site/**`; deliberately advisory rather than
  a required check, since a path-filtered required check that doesn't fire reads to GitHub as
  satisfied, not absent.
- **CI/deps hygiene** (#792): `backend/Dockerfile.prod`'s previously-unpinned `denoland/deno:bin`
  base image is now pinned; `.github/workflows/dependabot.yml` gained a `docker` entry for the
  root directory, so images pinned directly in the root compose files (postgres, minio, redis,
  opensearch) are now watched for updates; the `nvidia/pytorch` Dependabot ignore's stated reason
  was stale and has been corrected to the constraint that is actually still true (a base-image
  bump needs validation on real Blackwell hardware, which CI cannot do).
- **Docs homepage comparison table and hero copy re-verified against the shipped product**
  (#758): every claimed row was re-checked against the current codebase rather than assumed —
  nothing claimed is gated behind a default-off flag, and nothing claims multi-replica/horizontal
  backend scaling (which is unsupported). The hero tagline was rewritten from a feature list to
  an outcome-and-effort framing.
- **Coverage documentation correction finished** (#789): an earlier PR corrected two of four
  sites describing backend test coverage as "report-only" but missed
  `docs/CONTRIBUTING.md`, which still claimed no enforced threshold existed —
  `backend/pyproject.toml`'s `fail_under = 37` is in fact a hard, enforced (ratcheting) gate.
  Corrected; does not change the coverage floor itself.
...
- **Locale bundle no longer ships every language to every visitor**: locale data was
...
- **The role invariant was not actually enforced.** The constraints added in v369 make the
  ...
  role at all and satisfy the very check meant to prevent it. Migration `v377` makes the column
...
- **Revoking a share could still serve the revoked file's content from the chat retrieval cache**
  for up to 5 minutes (the cache TTL). Reproduced live: grant access → query (chunks present) →
  revoke → an identical re-query returned `cache_hit=True` with the revoked file's chunks still
  in the payload. The retrieval cache is deliberately ACL-blind (it keys on query/scope/settings/
  corpus-version, not on the caller's access), so an ACL change must invalidate it by bumping the
  corpus version — nothing did. Fixed at the one place every ACL rewrite already passes through:
  `update_file_access_index` now bumps the retrieval cache's corpus version after it finishes
  rewriting `accessible_user_ids`, including on partial failure. An endpoint-side bump was
  considered and rejected — the reindex is asynchronous, so bumping at the endpoint races the
  in-flight rewrite and can re-cache the revoked content for a fresh full TTL. **No operator
  action needed**; this closes automatically for all future share/permission changes.
- **Removing a user from a group via SCIM or an OIDC/LDAP identity-provider mapping did not
  update that group's shared files' search/chat visibility — indefinitely, not just for the
  cache TTL.** `update_file_access_index` (the task that rewrites which users can find or chat
  about a file via search) was dispatched only from the admin-UI group-membership endpoints; the
  SCIM group-membership service and the IdP group-mapping reconciler never dispatched it at all.
  A user deprovisioned from a group by their identity provider kept full search and RAG-chat
  visibility of every file shared with that group until some unrelated write happened to touch
...
configuration is renamed automatically by migration `v379`.
...
- **The OIDC configuration surface is renamed, and existing setups keep working.** Your
  ...
  renamed automatically by migration `v379`. **No identity-provider reconfiguration is needed**:
