...
class PermissionService:
    ...
    # ------------------------------------------------------------------
    # Documents (v400, #362 lane C3-remainder) — the direct-share counterpart
    # of the file methods above. A document has no collection concept, so
    # these read DocumentShare directly instead of joining through a
    # CollectionMember/CollectionShare pair. Field/behaviour shape matches
    # get_file_permission / get_accessible_file_ids_subquery / check_file_access
    # / get_users_with_file_access deliberately, so a document is the file
    # rule's obvious sibling rather than a new design.
    # ------------------------------------------------------------------
    ...
    def get_document_permission(
        ...
        *,
        ...
        """Get highest permission level for user on a document.

        Checks ownership, then direct and group ``DocumentShare`` grants.
        ``organization_id`` is a default-deny tenant gate, same contract as
        :meth:`get_file_permission`: a document outside the active scope
        resolves to ``None`` even via the sharing path.
        """
        owner_row = (
            db.query(Document.user_id, Document.organization_id)
        ...
        max_perm = (
            db.query(
                func.max(
                    case(
                        (DocumentShare.permission == "editor", 2),
                        (DocumentShare.permission == "viewer", 1),
                        else_=0,
                    )
                )
            )
            .filter(
                ...
                or_(
                    DocumentShare.target_user_id == user_id,
                    DocumentShare.target_group_id.in_(user_group_ids),
                ),
            )
            .scalar()
        )
    ...
    def check_document_access(
        ...
        *,
        ...
        """Check user has minimum permission on a document, raise 403 if not."""
        from fastapi import HTTPException

        permission = PermissionService.get_document_permission(
            db, document_id, user_id, organization_id=organization_id
        )
        if permission is None:
            raise HTTPException(
                status_code=403,
        ...
        if PERMISSION_LEVELS[permission] < PERMISSION_LEVELS[min_permission]:
            raise HTTPException(
                status_code=403,
                detail=f"Requires {min_permission} permission on this document",
            )
        return permission

    @staticmethod
    def get_accessible_document_ids_subquery(
        db: Session, user_id: int, *, organization_id: OrgScope = UNSCOPED
    ):
        """Return a subquery of ``Document`` IDs the user can access (owned + shared).

        Same shape as :meth:`get_accessible_file_ids_subquery`, without the
        collection indirection: a document share names the document directly.
        """
        user_group_ids = (
            select(UserGroupMember.group_id)
            .where(UserGroupMember.user_id == user_id)
        ...
        if not isinstance(organization_id, _Unscoped):
            if organization_id is not None:
                org_pred = Document.organization_id == organization_id
            else:
                org_pred = Document.organization_id.is_(None)

        owned_where = [Document.user_id == user_id]
        if org_pred is not None:
            owned_where.append(org_pred)
        owned = select(Document.id).where(*owned_where)
        ...
        if org_pred is not None:
            shared_stmt = select(DocumentShare.document_id).join(
                Document, Document.id == DocumentShare.document_id
            )
            share_where.append(org_pred)
        else:
            shared_stmt = select(DocumentShare.document_id)

        shared = shared_stmt.where(*share_where)

        return union_all(owned, shared).subquery()

    @staticmethod
    def get_users_with_document_access(db: Session, document_id: int) -> list[int]:
        """Get all user IDs who have access to a document (owner + direct/group shares).

        Used by ``app/tasks/search_indexing_task.py::_document_accessible_user_ids`` to
        compute the ``accessible_user_ids`` grant list a share change must reindex — the
        document counterpart of :meth:`get_users_with_file_access`.
        """
        document = db.query(Document).filter(Document.id == document_id).first()
        if not document:
            return []

        user_ids = {document.user_id}
