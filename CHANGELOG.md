...
- **Frontend modularity, quality & accessibility overhaul (issue #174)**: split the eight oversized Svelte components (`TranscriptDisplay`, `SettingsModal`, the file-detail / speakers / gallery routes, `Navbar`, `UserFileStatus`, `CollectionsPanel` — each 1,400–3,500 lines) into focused single-responsibility children under new `transcript/`, `settings/`, `speakers/`, `gallery/`, `navbar/`, `fileStatus/`, `collections/`, and `fileDetail/` folders, with each route/parent kept as a thin coordinator and **no behavior, DOM, or visual change**. Consolidated ~25 duplicated time formatters into `$lib/utils/formatting` (test-locked), extracted the client-side transcript export into a golden-tested `$lib/export` module, deduplicated the collections create/edit modals, and added reusable, accessibility-correct UI primitives (`Tabs`, `Dropdown`, `Avatar`, `Badge`, `ExpandableSection`) plus a typed `clickOutside`/`apiError`/`focusTrap` toolkit. Stood up a **Vitest** unit/component harness (61 tests) and wired **ESLint** (flat config) into pre-commit + CI; added a SvelteKit `+error.svelte` boundary, modal focus-trap / `aria-modal` / return-focus, icon `aria-label`s, and 16 per-folder `CLAUDE.md` docs. Verified per-commit by svelte-check (0 errors/0 warnings), `vite build`, the unit suite, and the live Playwright E2E suite.
...
- **Frontend production hardening (issue #174)**: a `dependency-audit` CI job (`npm audit` + `pip-audit`); the theme bootstrap externalized from `app.html` to a static `/theme.js` (prerequisite for dropping `script-src 'unsafe-inline'`); server-side enforcement of the speaker-label length cap (`SpeakerUpdate` `max_length`); and a documented AWS production / hardening guide (`docs/deployment/AWS_PRODUCTION.md`) capturing the security audit (no bundle secrets, no prod source maps, DOMPurify on all `{@html}`, server-side authz) and the remaining CSP lockdown steps.
...
- **Database migrations** `v360_add_file_pipeline_timing`, `v361_add_media_file_imohash`, `v362_add_pipeline_timing_markers`, `v363_add_asr_access_key_id`, and `v364_add_content_redaction` apply automatically on backend startup (idempotent, additive). No manual `alembic` step is required in dev.
