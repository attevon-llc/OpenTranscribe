...
#: The mount every splitter-dependent worker needs, by container path.
NLTK_CACHE_PATH = "/home/appuser/.cache/nltk_data"
...
def test_every_queue_that_chunks_has_a_worker_with_punkt() -> None:
    ...
    for queue, task_name in sorted(queues.items()):
        ...
        if not serving:
            ...
            continue
        for name in serving:
            if NLTK_CACHE_PATH not in _mount_targets(services[name]):
    ...
    assert not unmounted, (
        f"These workers chunk transcripts with no {NLTK_CACHE_PATH} mount, so they cut "
