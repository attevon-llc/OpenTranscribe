...
def main(argv: list[str] | None = None) -> int:
    ...
    # above, so a top-level import raises E402 — and a per-NAME suppression cannot
    # clear it, because ruff reports E402 against the whole statement rather than the
    # names inside it. Relocating a suppression until the tool honours it leaves the
    # code unchanged and only the tool quieter; moving the import to its single caller
    # removes the finding outright. This function already does the same for
    # `index_reader` and `celery_app`, for the same reason.
    ...
    if not args.no_dispatch:
        ...
        since = (datetime.datetime.now(datetime.UTC) - datetime.timedelta(seconds=1)).isoformat()
    ...
    logger.info(
        'Settled: %d files, %d chunks in %.0fs', settled['files'], settled['chunks'], elapsed
    )
    return 0
