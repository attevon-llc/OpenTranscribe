...
- **Optional transcript context**: any conversation can turn retrieval off and act as a plain assistant, with an unmistakable *Context off* chip so an ungrounded answer never looks like a grounded one. Three-layer system prompt (immutable base rules → per-user default from Settings → Chat → per-conversation override) where user layers can only append, never replace the base rules.
