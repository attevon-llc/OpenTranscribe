...
Migration `v394_add_document_tables` adds `document` and `document_chunk` as their own tables —
...
`watch_source_file.document_id` (migration `v395`, `ON DELETE SET NULL`, mirroring
