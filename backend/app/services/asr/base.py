...
def normalize_speaker_label(label: str | int | None) -> str | None:  # noqa: C901
    """Normalize any speaker label format to 0-indexed SPEAKER_XX.

    Module-level so both the ASR and diarization provider hierarchies can share
    one implementation without either needing an instance of the other's ABC.
    ...
    * ``"spk_0"``, ``"spk_1"`` … (0-indexed, AWS Transcribe)
    ...
    *before* calling this function (subtract 1, then pass the resulting
    0-indexed integer or ``"SPEAKER_XX"`` string).  See ``google_provider.py``.

    Args:
        label: Raw provider speaker label, or None.

    Returns:
        Canonical ``SPEAKER_XX`` string, or None when *label* is None.
    """
    if label is None:
        return None
    label_str = str(label).strip()

    # Already SPEAKER_-prefixed. Re-pad the numeric suffix so "SPEAKER_1" and
    # Gladia's "speaker_0" land on the same canonical 2-digit form every other
    # branch emits; leave non-numeric suffixes (e.g. SPEAKER_UNKNOWN) untouched.
    if label_str.upper().startswith("SPEAKER_"):
        suffix = label_str[len("SPEAKER_") :]
        if suffix.isdigit():
    ...
    m = re.fullmatch(r"[Ss](\d+)", label_str)
    if m:
    ...
    m = re.fullmatch(r"spk_(\d+)", label_str, re.IGNORECASE)
    if m:
        return f"SPEAKER_{int(m.group(1)):02d}"

    # "Guest-N" (Azure ConversationTranscriber) — 1-indexed.
    m = re.fullmatch(r"[Gg]uest-(\d+)", label_str)
    if m:
        return f"SPEAKER_{max(int(m.group(1)) - 1, 0):02d}"
...
def sanitize_provider_error(message: str, api_key: str | None = None) -> str:
    """Strip API keys and credential-like tokens from an error message.

    Module-level twin of :func:`normalize_speaker_label` — shared by the ASR and
    diarization hierarchies so the scrubbing rules cannot drift apart.
    ...
    3. Regex removal of known key prefixes (``sk-``, ``dg_``, ``aai_``, …).

    Args:
        message: Raw error message.
        api_key: The key to redact by exact match, if known.

    Returns:
        The message with credential-like tokens replaced by ``***``.
    """
    if not message:
        return message
...
class ASRProvider(ABC):
    ...
    def _normalize_speaker_label(self, label: str | int | None) -> str | None:
        ...
        Thin delegate to :func:`normalize_speaker_label` — see that function for
        the full list of handled formats and their base indices.
        """
    ...
    def _sanitize_error(self, message: str, api_key: str | None = None) -> str:
        ...
        Thin delegate to :func:`sanitize_provider_error`.
        """
