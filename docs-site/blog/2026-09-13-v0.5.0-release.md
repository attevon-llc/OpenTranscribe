...
**This release contains breaking changes and action-required upgrade steps.** Read [Upgrade Notes](/blog/v0.5.0-release#upgrade-notes) before pulling.
...
- **Stopping a GPU worker mid-transcription is still not safe.** Workers now release the GPU on shutdown and get a measured 30-second grace period instead of being killed after 10, so an idle or between-stages stop is clean. But an in-flight transcription cannot be interrupted: the worker pool waits for the running task to return, so it still runs to the ceiling and is killed. Releasing sooner would free VRAM out from under a live CUDA kernel, which is worse. Let a transcription finish before stopping the stack. Tracked as [#809](https://github.com/attevon-llc/OpenTranscribe/issues/809).
...
The live view is the [roadmap](/roadmap), generated from GitHub issues rather than hand-maintained, so it can't drift from what we're actually doing. The reasoning behind each release — what it's for and when it's done — is in [Release Themes](/docs/developer-guide/roadmap). If there's something specific you want, open an issue; that's genuinely how most of this release got decided.
