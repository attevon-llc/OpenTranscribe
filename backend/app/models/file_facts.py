...
- #362 makes documents rows in ``media_file`` with ``kind='document'``, so the sidecar is
  already the *document* analog #403 comment 1 (Nuance 3) asks for — same table, same
  ``facts``/``digest``/``keyphrases`` shape, ``char_range`` provenance instead of
  ``segment_ids``. No second table, no second code path.
