...
\$966--\$5,350 for equivalent cloud transcription. A latest-code validation on a
mixed 58\,h corpus (Sec.~\ref{sec:mixedcorpus}) further shows dual-A6000 throughput of
\textbf{81.3$\times$} (58\,h cleared in $\approx$43\,min) and characterises the
batch-scheduling and per-file latency behaviour of heterogeneous real-world workloads.
\end{abstract}
...
\subsection{Full Mixed-Corpus Validation (Latest Code, 2026)}
\label{sec:mixedcorpus}

The scaling results above (Table~\ref{tab:scaling}) used five \emph{duration-matched}
files ($\approx$2.73\,h each) to isolate pure GPU concurrency behaviour. To validate the
latest pipeline under a realistic, heterogeneous workload, we re-ran the benchmark on a
\textbf{40-file, 58.0\,h corpus} spanning four duration tiers (5\,min to 4.7\,h), uploaded
as a batch through the production API. Both result sets are retained here for direct
comparison.

\begin{table}[!t]
\centering
\caption{Single-A6000 Sweep --- Mixed 58\,h Corpus (Latest Code)}
\label{tab:mixed}
\begin{tabular}{@{}rrrrr@{}}
\toprule
\textbf{Wkr} & \textbf{Thru-put} & \textbf{Speedup} & \textbf{VRAM} & \textbf{GPU} \\
             & \textbf{(agg)}    & \textbf{vs c1}   & \textbf{Peak} & \textbf{Util} \\
\midrule
1  & 39.0$\times$ & 1.00$\times$ & \num{5239}\,MB  & 54\% \\
\textbf{4}  & \textbf{45.9$\times$} & \textbf{1.18$\times$} & \num{35883}\,MB & 85\% \\
8  & 39.2$\times$ & 1.01$\times$ & \num{48287}\,MB & 90\% \\
10 & 39.2$\times$ & 1.01$\times$ & \num{44279}\,MB & 91\% \\
12 & 39.5$\times$ & 1.01$\times$ & \num{46423}\,MB & 92\% \\
16 & 30.8$\times$ & 0.79$\times$ & \num{46921}\,MB & 93\% \\
\bottomrule
\end{tabular}
\\[4pt]
{\footnotesize Aggregate throughput peaks at conc=4 (45.9$\times$) and declines beyond,
  in contrast to the duration-matched run (Table~\ref{tab:scaling}: linear to conc=12,
  54.6$\times$). Reconciled below.}
\end{table}

\paragraph{Reconciling the two corpora.}
The duration-matched run scales linearly to twelve workers; the mixed corpus peaks at four
and declines. \emph{These are not contradictory}---they measure different regimes. With
matched files, all concurrent workers finish together and the GPU stays saturated. With a
mixed batch, the short files drain quickly and workers then idle waiting on the few long
files: a tail-limiting effect of batch scheduling, \emph{not} a GPU compute ceiling (VRAM
still plateaus at the same $\approx$48\,GB, confirming the shared single-model design). The
operating implication: \textbf{conc=6--8 for sustained, continuously-fed queues}
(Table~\ref{tab:scaling}); \textbf{conc=4 for bursty, mixed-length uploads}
(Table~\ref{tab:mixed}). \emph{[Two corpora retained for later reconciliation; values not
yet publication-final.]}

\paragraph{Per-file latency under load.}
The mixed-corpus run also captures the multi-user latency tradeoff (Table~\ref{tab:latency}):
an individual file's GPU-stage time inflates 3.9$\times$ at conc=4 and 6--12$\times$ at
conc=8+ relative to the isolated (conc=1) baseline, because the shared GPU time-slices
across requests. Per-file throughput nonetheless stays above realtime ($\geq$4.4$\times$) at
all tested concurrencies. Latency-sensitive multi-user deployments should keep per-GPU
concurrency modest and scale horizontally rather than stacking concurrency onto one card.

\begin{table}[!t]
\centering
\caption{Per-File Latency \& Contention vs.\ Concurrency (Mixed Corpus)}
\label{tab:latency}
\begin{tabular}{@{}rrr@{}}
\toprule
\textbf{Workers} & \textbf{GPU Inflation vs c1} & \textbf{Per-File RTF (p50)} \\
\midrule
1  & 1.0$\times$  & 39.3$\times$ \\
4  & 3.9$\times$  & 12.0$\times$ \\
8  & 11.6$\times$ & 5.2$\times$  \\
12 & 8.6$\times$  & 5.2$\times$  \\
16 & 6.3$\times$  & 4.4$\times$  \\
\bottomrule
\end{tabular}
\\[4pt]
{\footnotesize GPU inflation = a file's GPU-stage time vs the conc=1 isolated baseline.
  Per-file RTF = audio duration $/$ GPU-stage time.}
\end{table}

\paragraph{Dual-GPU throughput.}
Running both A6000s (GPU\,0 + GPU\,2) at conc=4 each clears the full 58\,h corpus in
\textbf{$\approx$43\,minutes} at \textbf{81.3$\times$ aggregate realtime}---\textbf{1.77$\times$}
the single-card mixed-corpus result (45.9$\times$). The sub-linear factor reflects the shared
CPU preprocessing stage feeding both GPUs (CPU averaged 1490\%, $\approx$15 cores); per-card
VRAM peaked at \num{35739}\,MB. This confirms the architecture scales across physical GPUs
with near-linear throughput.
...
% ==============================================================================
% VIII. COMPARISON TO COMMERCIAL CLOUD APIS
% ==============================================================================
\section{Comparison to Commercial Cloud APIs}

The preceding sections establish on-premises throughput. The complementary
question is \emph{accuracy}: does an offline open-source stack match the
commercial cloud APIs it replaces? We benchmarked the local engine against five
diarizing cloud providers on a hand-labeled, two-speaker public interview clip
(10\,min), scoring \emph{word speaker error rate} (WSER)---the fraction of words
assigned the wrong speaker after optimal label permutation, measured against
human ground-truth turns---alongside end-to-end realtime factor.

\subsection{Accuracy and Latency}

Each engine carries its own word inventory, so every engine's words are
midpoint-mapped to the same reference turns before scoring; the comparison is
therefore apples-to-apples on speaker attribution. Local timing is warm-model GPU
processing; cloud timing is end-to-end API latency (upload, provider queue,
inference, and result retrieval). All cloud keys were configured through the
application and encrypted at rest.

\begin{table}[!t]
\centering
\caption{Local vs.\ Commercial Cloud (10\,min, two speakers, WSER vs.\ human labels)}
\label{tab:cloudcompare}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Engine} & \textbf{WSER} & \textbf{Realtime} \\
\midrule
\textbf{Local (smoothed)}        & \textbf{0.27\%} & \textbf{44.2$\times$} \\
Gladia \texttt{standard}         & 0.27\%          & 22.4$\times$ \\
AssemblyAI \texttt{universal-3-pro} & 0.49\%       & 36.0$\times$ \\
AWS Transcribe \texttt{standard} & 0.69\%          & 9.7$\times$ \\
pyannote.ai \texttt{precision-2} & 0.70\%          & 27.6$\times$ \\
Deepgram \texttt{nova-3}         & 5.75\%          & 143.3$\times$ \\
\midrule
Local (uncorrected)             & 1.15\%          & 44.2$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
The local stack \emph{ties for the best accuracy of any engine} (0.27\% WSER),
outscoring premium pyannote.ai (0.70\%), AWS Transcribe (0.69\%), AssemblyAI
(0.49\%), and Deepgram (5.75\%), at 44$\times$ realtime---offline and at zero
per-minute cost. The boundary-correction post-processing is
responsible for the local improvement from 1.15\% to 0.27\%, with no added
wall-clock time. No cloud provider eliminates turn-boundary errors: even
\texttt{precision-2} emits short wrong-speaker islands, confirming the boundary
defect is universal rather than a local-only artifact. AWS Transcribe achieves
competitive accuracy but the worst latency (9.7$\times$), as its batch model
requires an S3 round-trip plus an asynchronous job queue. These figures are a
single-clip, point-in-time snapshot (cloud models are versioned and change); the
local pipeline is the only deterministic, reproducible baseline.

% ==============================================================================
% IX. CAPACITY PLANNING
% ==============================================================================
...
covered by ${\sim}20$ chunks; at 70\% overlap it's only 6, and if a few of
