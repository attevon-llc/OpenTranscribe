...
def _get_owned_document(db: Session, document_uuid: UUID, current_user: User) -> Document:
    """404 (never 403) on a document that exists but is not this user's.

    Documents have no sharing/collections yet (v1 scope) — every document is owner-only
    or admin-visible, so there is no "exists but you can't see it" case worth
    distinguishing from "does not exist"; the tag plane's `_writable_tag_ids` reasoning
    for 404-not-403 (`app/api/CLAUDE.md`) applies for the same reason: a 403 confirms the
    id refers to something real, which is enumerable.
    """
...
async def upload_document(
    ...
    storage_path = f"documents/user_{current_user.id}/document_{doc.id}/{sanitized}"
...
async def list_documents(
    ...
    """List the caller's documents, newest first. Owner-scoped only — see
    :func:`_get_owned_document` for why there is no sharing to account for yet.
    """
...
async def get_document(
    ...
    doc = _get_owned_document(db, document_uuid, current_user)
    return _to_response(doc)
