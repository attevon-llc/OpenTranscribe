...
def test_detection_arm_returns_v372_or_later_on_current_schema(db_session, revisions_at_or_after):
    """An untracked DB with the current schema stamps at v372 or a later
    revision that revises it (only the newest revision can pin an exact value —
    it does so in its own ``test_v3NN_migration_consistency.py``)."""
