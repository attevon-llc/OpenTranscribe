...
def add_tags_to_file(db: Session, file_id: int, tag_names: list[str], user_id: int) -> bool:
    """
    ...
    Args:
        ...
        user_id: Owner for any tag this creates. Omitting it would create an
            ownerless (system) tag, publishing the name to every account.
    ...
    try:
        for tag_name in tag_names:
            # Get or create tag, scoped to this user's vocabulary
            tag, created = get_or_create(db, Tag, name=tag_name, user_id=user_id)
...
def remove_tags_from_file(db: Session, file_id: int, tag_names: list[str]) -> bool:
    ...
    """
    try:
        # Resolve names through THIS file's attachments. Tag names are only
        # unique per owner, so a bare name lookup would sweep in other users'
        # identically-named tags.
        tag_ids = (
            db.query(Tag.id)
            .join(FileTag, FileTag.tag_id == Tag.id)
            .filter(FileTag.media_file_id == file_id, Tag.name.in_(tag_names))
            .all()
        )
